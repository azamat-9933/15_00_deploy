...
is_in_home = openapi.Parameter(
    'home',
    openapi.IN_QUERY,
    description="This query parameter is used to filter data that shown in the home page",
...
class ActionListView(ListAPIView):
    queryset = Action.objects.filter(expires_in__gt=datetime.date.today()).order_by('-expires_in')
...
class FeedbackCreateListAPIView(CreateAPIView, ListAPIView):
    ...
    def get_queryset(self):
        if self.request.method == 'GET':
            return super().get_queryset().filter(is_view=True)
